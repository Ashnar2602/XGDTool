...
internal class God : Base
{
    ...
    private readonly uint TotalDataBlocks = 0;

    public override Format ImageFormat => Format.GOD;
    public override uint TotalSectors => XDVDFS.SectorCount(TotalDataBlocks * GOD.BLOCK_SIZE);
    ...
    public God(IReadOnlyList<string> files) : base(files)
    {
        ...
        TotalDataBlocks = (uint)totalDataBlocks;
    }
    ...
    private (FileStream stream, long offset, long remaining) GetStreamForSector(uint sector)
    {
        ...
        newOffset += (sector * XDVDFS.SECTOR_SIZE) % GOD.BLOCK_SIZE; // Add offset within data block

        // Just provide remaining bytes in the current block for simplicity
        // can be revisited later
        long remaining = GOD.BLOCK_SIZE - ((sector * XDVDFS.SECTOR_SIZE) % GOD.BLOCK_SIZE);
