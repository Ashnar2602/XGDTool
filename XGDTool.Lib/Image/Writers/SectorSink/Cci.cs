...
internal class Cci(IWriterOptions options, Title.Info titleInfo) : ISectorSink
{
    ...
    private readonly List<FileEntry> OutFiles = [];
    ...
    public async Task<List<string>> FinalizeImage(IProgress<Converter.Progress>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (var file in OutFiles)
            {
                ...
                ulong uncompressedSize = (ulong)file.IndexEntries.Count * XDVDFS.SECTOR_SIZE;
                ...
                file.Stream.Write(header.Serialize());
                file.Stream.Dispose();
            }
    ...
    private void CompressAndWriteSectors(uint startSector, ReadOnlyMemory<byte> buffer, CancellationToken ct = default)
    {
        ...
        int sectorCount = buffer.Length / XDVDFS.SECTOR_SIZE;
        ...
        try
        {
            ...
            i =>
            {
                ...
                int maxCompressedSize = LZ4Codec.MaximumOutputSize(XDVDFS.SECTOR_SIZE);
                ...
                try
                {
                    ...
                    var isCompressed =
                        compressedSize > 0 &&
                        compressedSize < (XDVDFS.SECTOR_SIZE - (4 + alignMult));
    ...
    private static FileStream CreateOutputStream(string path)
    {
        ...
        stream.Seek(CCI.Header.SIZE, SeekOrigin.Begin);
        return stream;
    }
}
