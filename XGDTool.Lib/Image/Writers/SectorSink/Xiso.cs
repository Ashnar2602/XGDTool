...
internal class Xiso(IWriterOptions options, Title.Info titleInfo) : ISectorSink
{
    ...
    public async Task WriteSectorsAsync(uint startSector, ReadOnlyMemory<byte> buffer, CancellationToken ct = default)
    {
        if (!XDVDFS.IsSectorAligned(buffer.Length))
            throw new ArgumentException($"Buffer length must be a multiple of {XDVDFS.SECTOR_SIZE}", nameof(buffer));
    ...
    public async Task<List<string>> FinalizeImage(IProgress<Converter.Progress>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!XDVDFS.IsSectorAligned(totalWritten))
            ...
            return [..Streams.AsEnumerable().Select(s => s.Name)];
        }
        finally
        {
            WriteLock.Release();
    ...
    {
        var offset = XDVDFS.SectorToOffset(sector);

        if (!Split || offset < XDVDFS.SPLIT_MARGIN)
            return (GetOrCreateStream(0), XDVDFS.SectorToOffset(sector));

        return (GetOrCreateStream((int)(offset / XDVDFS.SPLIT_MARGIN)), offset % XDVDFS.SPLIT_MARGIN);
    }
    ...
    private FileStream GetOrCreateStream(int index)
    {
        ...
        for (int i = Streams.Count; i <= index; i++)
        {
            ...
            if (i < index)
                stream.SetLength(XDVDFS.SPLIT_MARGIN);
