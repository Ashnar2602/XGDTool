...
internal class God(IWriterOptions options, Title.Info titleInfo) : ISectorSink
{
    ...
    private void WriteLiveHeader(byte[] finalMhtHash)
    {
...

        {
            ...
            // ms.Write(xexInfo.Serialize(), 0, XEX.ExecutionInfo.SIZE);
            TitleInfo.XexExecutionInfo
                .Serialize()
                .CopyTo(headerBuf.AsSpan(0x354, XEX.ExecutionInfo.SIZE));
            ...
            BinaryPrimitives.WriteUInt32BigEndian(headerBuf.AsSpan(0x344, 4), contentType);

            // ms.Seek(0x37D, SeekOrigin.Begin);
            // // ms.Wri
            // ms.Write(finalMhtHash, 0, finalMhtHash.Length);
            finalMhtHash.CopyTo(headerBuf.AsSpan(0x37D, finalMhtHash.Length));

            // ms.Seek(0x3A0, SeekOrigin.Begin);
            BinaryPrimitives.WriteUInt32LittleEndian(headerBuf.AsSpan(0x3A0, 4), partCount);
            BinaryPrimitives.WriteUInt32BigEndian(headerBuf.AsSpan(0x3A4, 4), partsWrittenSize);
        ...
        }
        {
            ...
            // ms.Write(titleNameBytes, 0, titleNameSize);
            titleNameBytes.CopyTo(headerBuf.AsSpan(0x411 + 1, titleNameSize));
            titleNameBytes.CopyTo(headerBuf.AsSpan(0x1691 + 1, titleNameSize));
        }
        if (TitleInfo.TitleIconData != null && TitleInfo.TitleIconData.Length > 0)
        {
            ...
            // ms.Write(BitConverter.GetBytes(titleIconSize), 0, 4);
            BinaryPrimitives.WriteUInt32BigEndian(headerBuf.AsSpan(0x1712, 4), (uint)TitleInfo.TitleIconData.Length);
            BinaryPrimitives.WriteUInt32BigEndian(headerBuf.AsSpan(0x1716, 4), (uint)TitleInfo.TitleIconData.Length);
            ...
            // ms.Write(TitleInfo.TitleIconData, 0, TitleInfo.TitleIconData.Length);
            TitleInfo.TitleIconData.CopyTo(headerBuf.AsSpan(0x171A, TitleInfo.TitleIconData.Length));
            TitleInfo.TitleIconData.CopyTo(headerBuf.AsSpan(0x571a, TitleInfo.TitleIconData.Length));
        }
        {
            ...
            headerHash.CopyTo(headerBuf.AsSpan(0x32C, SHA1.HashSizeInBytes));
        }
    ...
    {
        long blockNum = (sector * XDVDFS.SECTOR_SIZE) / GOD.BLOCK_SIZE;
        ...
        newOffset += (sector * XDVDFS.SECTOR_SIZE) % GOD.BLOCK_SIZE;
